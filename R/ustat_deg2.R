ustat_deg2 <- function(x, psi){## compute degree k=2 U-statistic based on (n x p) data matrix "x" and
    ...
    ## thus there are a total of n(n-1)(n-2) products involved.  the covariance is computed
    ## by the mean of the relevant products minus the square of the marginal mean of psi,
    ## all times (n)/(n-1) to make it closer to the unbiased estimator of the true covariance.
    ## we estimate the variance of psi using a similar degrees of freedom correction.
    ss2     <- (mean(psivals^2) - U^2) * (n/(n-1))
    ss1     <- 0.0
    ind     <- 0L
    for(istar in 1:(n-1)){
        for(jstar in (istar+1):n){
            ...
            ss1 <- ss1 + sum(psivals[ind] * psivals[others])
        }
    }
    ss1  <- ((ss1 / (n * (n-1) * (n-2))) - U^2) * (n/(n-1))
